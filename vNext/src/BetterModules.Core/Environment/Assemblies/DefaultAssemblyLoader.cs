...
namespace BetterModules.Core.Environment.Assemblies
{
    ...
    public class DefaultAssemblyLoader : IAssemblyLoader
    {
        ...
        /// </summary>
        public DefaultAssemblyLoader(ILoggerFactory loggerFactory)
        {
            loadedAssemblies = new ConcurrentDictionary<string, Assembly>();
            logger = loggerFactory.CreateLogger(typeof (DefaultAssemblyLoader).FullName);
        }
        ...
        public IEnumerable<Type> GetLoadableTypes(Assembly assembly)
        {
            ...
            {
                logger.LogWarning("Failed to scan loadable types from assembly {0}.", ex, assembly.FullName);
